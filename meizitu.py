...
def download_imgs(page1_url,fileNameEncode):
    ...
    if not os.path.exists(pic_path):
        os.mkdir(pic_path.encode(fileNameEncode))
    ...
    if not os.path.exists(local_path):
        try:
            os.mkdir(local_path.encode(fileNameEncode))
        except:
            pass
    if img_urls is None or len(img_urls) == 0:
        ...
        for link in img_urls:
            ...
            # req.add_header('User-Agent',"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36")
